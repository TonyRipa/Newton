...
For increased numerical stability, the differential transform does not use real linear stencils (like $c_1 f(x+3h) + c_2 f(x+2h) + c_3 f(x+h) + c_4 f(x)$ [or even a centered version]), but instead uses complex circular stencils (like $c_1 f(x+h)+c_2 f(x+ih)+c_3 f(x-h)+c_4 f(x-ih)$ ) . Their stencil weight coefficients (c1,c2,...) are obtained from the DFT Matrix, as are the sample points (i.e. the coefficients of h). We take h to be a small positive real, like .01 . Taking h=1 would give just the DFT, which is not actually as good an approximant of the derivatives. These Fourier kind of roots of unity stencils, give far more decimal places of accuracy than their real linear counterpart.
